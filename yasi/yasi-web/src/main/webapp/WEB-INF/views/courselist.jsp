...
<div class="head">
  ...
  <input name="cour" id="cour" type="text" class="sear_inp" value="${cour}" onFocus="if (value =='${cour}'){value =''}" onBlur="if (value ==''){value='请输入课程名称'}"/><input type="button" class="sear_btm" value="搜索" onclick="search()"/>
   </form>
   </c:if>
   <c:if test="${empty cour}">
  <form  id="searchform" action="${ctx}/course/search">
...
<div id="has-divider ">
     ...
     <c:forEach items="${typeList}" var="type" varStatus="status">
      ...
      <c:forEach items="${courseList}" var="se" varStatus="status">
        <li><a>&nbsp; ${se.cName}</a> <input type="button" id="add" value="立即报名" />

           <input type="button" id="talk" value="在线咨询" /></li>
